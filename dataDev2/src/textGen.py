...
class TextGen:
    ...
    def applicationsDB_Gen(self, config):
        ...
        if config['JSON_pairs']['enabled']:
            ...
            f_df1.to_csv(os.path.join(os.getcwd() + config['output_dir']), index=False)
        
        
        
        # 2 - General & Contextual QA pairs 
        if config['general_pairs']['enabled']:
            ...
            # Iterate through case studies in df2
            for row in tqdm(df2.iterrows(), total=len(df2), desc="Processing applications", position=0, dynamic_ncols=True, colour='blue'):
                ...
                # Skip short texts and exclusions
                if self.bad_text(row[1]['text'], row[1]['file_name'], config['general_pairs']['bad_files']) == True:
                    continue

                # Iterate through text windows in each case study
                for i in tqdm(range((len(row[1]['text']) - config['general_pairs']['window_size']) // config['general_pairs']['stride']), desc=f"Processing {row[1]['file_name']}", position=1, leave=False, dynamic_ncols=True, colour='yellow'):
                    ...
                    # Randomly select pattern and get keys
    ...
    def definitionsDB_Gen(self, config):
        # 3 - Definition pairs based directly on D DB
        ...
        for row in tqdm(df.iterrows(), total=len(df), desc="Processing definitions", position=0, dynamic_ncols=True, colour='blue'):
            for _ in range(config['pairs_per_definition']):
                ...
                output_txt = config['output'][random.randint(0, len(config['output'])-1)].format(definition=row[1]['definition']).strip()
                id = str(uuid.uuid4())
    ...
    def keys_in_pattern(self, pattern) -> list:
